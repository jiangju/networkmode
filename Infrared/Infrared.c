...
void *PthreadInfraredRv(void *arg)
{
	...
	int len = 0;
	unsigned short i = 0;
	unsigned char usartBuffer[INFRARED_RD_LEN] = {0};
	int l = 0;
...
//	fcntl(_fd_infrared, F_SETFL, 0);
	while(1)
	{
		...
		usleep(10);
		memset(usartBuffer, 0, INFRARED_RD_LEN);
		printf("");	//去掉打印就会串口不接收，原因不明
		len = read(_fd_infrared, usartBuffer, INFRARED_RD_LEN);
		if(len == -1)
		{
			continue;
		}
...
//			printf("infrared: r  %d   w   %d\n",_InfaraedBuffer.ReadIndex,_InfaraedBuffer.WriteIndex);
...
//			printf("\n");
			...
			if(1 == rvframe3761.Frame376_1Link.CtlField.DIR)
			{
				continue;
			}
		}
