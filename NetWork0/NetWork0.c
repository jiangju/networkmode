	...
	while(1)
	{
		...
		if(true == rvframe3761.IsHaving)
		{
			...
			if(0 == ret)	//存在
			{
				...
				if(0 != check_hld_route_node_s_ter(s, ter))	//不是
				{
					//修改套接字对应的终端地址
		...
		for(i = 0; i < num; i++)
		{
			if(r_evt[i].data.fd == s)
			{
				...
				else
				{
					//将新连接的套接字加入到路由链表中
					feed_watch_dog(wdt_id);	//喂狗
					ret = check_hld_rv_s(rws);	//是否有该套接字的接收缓存
					if(0 != ret)	//没有
					{
						if(0 != add_hld_rv_s(rws))	//申请接收缓存失败
						{
							del_event(_epollfd, rws, EPOLLIN);		//路由链表添加节点失败
							close(rws);
							continue;
						}
					}
					ret = check_hld_route_node_s(rws);				//路由链表中是否存在rws
					if(0 == ret)									//存在
					{
						dele_hld_route_node_s(rws);					//关闭该套接字
						del_hld_rv_s(rws);							//删除接收缓存
						del_event(_epollfd, rws, EPOLLIN);
					}
					else
					{
						if(0 != add_hld_route_node(rws))				//添加路由节点
						{
							del_hld_rv_s(rws);						//删除接收缓存
							del_event(_epollfd, rws, EPOLLIN);		//路由链表添加节点失败
							close(rws);
			...
			else
			{
				...
				if(0 != ret)
				{
					if(0 != add_hld_rv_s(r_evt[i].data.fd))	//申请接收缓存失败
					{
						del_event(_epollfd, r_evt[i].data.fd, EPOLLIN);		//路由链表添加节点失败
				...
				/*将读取到的数据存入相应的接收缓存中*/
				if(0 == update_hld_rv_buff(r_evt[i].data.fd, buf, len))	//添加成功
				{
